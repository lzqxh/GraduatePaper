...
否则在图$G^*$上存在线形规划模型的约束条件不满足，而该条件在图$G$上也依然不满足，这与$x,y$作为最优解冲突。
...
同时既然我们已经有了关于图$G^*$上Dual-$LPVC(G)$的最优解$y^*$，
那么显然也可以在线性时间复杂度内构造出求解Dual-$LPVC(G^*)$的网络$W^*$的最大可行流。
...
对于求解图$G'$上Dual-$LPVC(G)$问题的网络$W'$，可以简单通过从原本网络W中删除点集$S_W$以及相应的边获得。
...
因为全$\frac{1}{2}$函数是图G的Primal-$LPVC(G)$最优解，所以网络W的最大可行流流量$F = 2 * val(x) = \abs{V}$。
