...
\begin{definition} [\text{W[1]-hard}] \label{def_whard}
...
已经证明了不可能存在核心化算法使得该问题的任意实例都能产生小于$O(ck)$的核，其中c是小于2的常数\upcite{chen2001vertex}。
...
在这个4个不同的参数化顶点覆盖版本，可以证明其中4个参数大小关系有$k \le \mu_1 \le \mu_2 \le \mu_3$。在表\ref{tab:vcfpt}中，我们列举了近几年中对于这几个参数化顶点覆盖问题的研究成果。
\begin {table}[H]
